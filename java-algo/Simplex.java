 ...
 *  Simplex.java -- 線形計画法
 *
 *  @version $Revision: 2.13 $, $Date: 2003/03/21 20:37:38 $
 */
...
public class Simplex {
    ...
    /** 正常終了 */
    private static void exitNormal(String message) {
        System.out.println(message);  System.exit(0);
    }

    /** 実数に変換 */
    private static double nextDouble(StringTokenizer t) {
        return Double.valueOf(t.nextToken()).doubleValue();
    }
    ...
    private static void readData() throws IOException {
        ...
        if (m < 1) exitError("条件の数 m が 1 未満です");
        if (n < 1) exitError("変数の数 n が 1 未満です");
    ...
    private static void printTableau() {  printTableau(-1, -1);  }
    ...
    /** 掃き出し */
    private static void pivot(int ipivot, int jpivot,
                                          double[] pivotColumn) {
    ...
    private static void minimize() {
        ...
        for ( ; ; ) {
            ...
            if (ipivot == 0) exitNormal("目的関数は下限がありません");
    ...
    private static void phase1() {
        ...
        minimize();
        if (tableau(0, 0) < -EPSILON) exitNormal("可能な解はありません");
        for (int i = 1; i <= m; i++)  // 人為変数から基底をなくす
            if (col[i] > n2) {
                int j;
                for (j = 1; j <= n; j++)
                    if (row[j] == 0
                        && Math.abs(tableau(i, j)) > EPSILON) {
    ...
    private static void phase2() {
        ...
        jmax = n2;
        for (int j = 0; j <= n; j++) a[0][j] = c[j];  // 目的関数
        minimize();
    }
