 ...
 *  Fukumen.java -- 覆面算
 *
 *  @version $Revision: 1.8 $, $Date: 2003/03/29 11:34:48 $
 */
...
public class Fukumen {
    ...
    public static void main(String[] args) throws IOException {
        ...
        HashMap letters = new HashMap();  // Character -> Letter の対応表
        for (int i = 0; i < row; i++) {
            int k = buffers[i].length();
            for (int j = 0; j < k; j++) {
                char c = buffers[i].charAt(k - 1 - j);
                Letter a = (Letter)letters.get(new Character(c));
                if (a == null) {  // 新規に Letter を追加
                    a = new Letter(c);  letters.put(new Character(c), a);
                }
                words[i][j] = a;
