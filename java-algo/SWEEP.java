...
* @version $Revision: 1.5 $, $Date: 2003/03/16 14:33:25 $
...
public class SWEEP {
    ...
    static void sweep(int k) { /* 掃き出し演算 */
        ...
        a[k][k] = 1 / d;
        if (added[k]) {  added[k] = false;  ndf++;  }
        else          {  added[k] = true;   ndf--;  }
    }

    static void regress(int k) { /* 基準変数 k について回帰係数などを出力 */
        if (added[k]) {  System.out.println("???");  return;  }
    ...
    */
    public static void main(String[] args) throws IOException {
        ...
        n = ndf = dataFile.numberOfRows();  m = dataFile.numberOfColumns();
        ...
        while (true) {
            ...
            int j = 0;
            if (c == 'X' || c == 'Y') {
                try { j = Integer.parseInt(s.substring(1));
