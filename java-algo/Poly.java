/**
* Poly.java -- 多項式の計算（複素数版）
*
* @version $Revision: 1.9 $, $Date: 2003/03/30 23:25:36 $
*/
...
public class Poly {
    ...
    static boolean isOdd(int n)   { return (n & 1) != 0; } // 奇数か
    static int alphabetOrder(int c) {  // aなら0, zなら25を返す
        return Character.digit((char)c, 36) - 10;
    }

    /** 新しいノードを作る */
    static Node newNode() {  return newNode(0, 0, null);  }
    ...
    /** ノードを消す */
    static void disposeNode(Node p) {  cells--;  }
    ...
    static Node copy(Node p) {
        ...
        while ((p = p.next) != null) {
            ...
            for (int i = 0; i < N_LETTER; i++) r.expo[i] = p.expo[i];
        }
        r.next = null;
        return q;
    }
