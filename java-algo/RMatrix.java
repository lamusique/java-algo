 ...
 *
 *  @version $Revision: 1.7 $, $Date: 2003/10/30 19:53:06 $
 */
public class RMatrix {
    ...
    public static void swapRow(double[][] mtx, int i, int j) {
        double[] temp = mtx[i]; mtx[i] = mtx[j]; mtx[j] = temp;
    }
    /**
    ...
    public static void swapCol(double[][] mtx, int i, int j) {
        for (int k = 0; k < mtx.length; k++) {
            double temp = mtx[k][i]; mtx[k][i] = mtx[k][j]; mtx[k][j] = temp;
        }
    }
    /**
     *  転置行列を返します。
    ...
    public static double[][] multiply(double[][] lhs, double[][] rhs) {
        ...
        if (l != rhs.length) throw new IndexOutOfBoundsException();
        double[][] ret = new double[n][m];
        for (int i = 0; i < n; i++) for (int j = 0; j < m; j++) {
            double value = 0.0;
            for (int k = 0; k < l; k++) value += lhs[i][k] * rhs[k][j];
            ret[i][j] = value;
        }
        return ret;
    }
    ...
    public static double[] multiply(double[][] mtx, double[] v) {
        ...
        for (int i = 0; i < n; i++) {
            double value = 0.0;
            for(int j = 0; j < m; j++) value += mtx[i][j] * v[j];
            ret[i] = value;
        }
        return ret;
    }
    ...
    public static double[] multiply(double[] v, double[][] mtx) {
        ...
        for (int i = 0; i < m; i++) {
            double value = 0.0;
            for(int j = 0; j < n; j++) value += v[j] * mtx[j][i];
            ret[i] = value;
        }
        return ret;
    }
...
/*****************************************************************************/
    ...
    public static void main(String[] args) {
        ...
        m1[0][0] = 1.0; m1[0][1] = 1.0; m1[1][0] = 1.0; m1[1][1] = 1.0;
        double[][]  m2 = (double[][])m1.clone();
        double[][]  m3 = add(m1,m2), m4 = multiply(m2,m3),
                    m5 = subtract(m3,m4);
