...
abstract public class FuncViewer extends Applet {
    //  描画領域のサイズ(ピクセル単位)
    private static int XMAX = 640, YMAX = 400;
    //  プロットを連結するかどうか
    private static boolean CONNECT_PLOTS = false;
    //  プロットの色
    private static Color PLOT_COLOR = Color.blue;
    //  軸の色
    private static Color AXES_COLOR = Color.black;
    //  グリッドを表示するかどうか
    private static boolean SHOW_GRID = false;
    //  グリッドの色
    private static Color GRID_COLOR = Color.lightGray;
    //  グリッドの間隔の最低の値(ピクセル単位)
    private static int MIN_GRIDSIZE = 20;
    //  オフスクリーンバッファ
    ...
    private static double[] parseRange(String param) {
        ...
        double min = Double.valueOf(token.nextToken()).doubleValue();
        if (!token.hasMoreTokens()) return null;
        double max = Double.valueOf(token.nextToken()).doubleValue();
