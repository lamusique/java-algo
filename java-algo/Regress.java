 ...
 *  最小２乗法による線形回帰分析
 *
 *  @version $Revision: 1.3 $, $Date: 2003/03/13 14:24:35 $
 */
public class Regress {
     ...
     *  @param  normsq  各列の２乗和
     */
    static int lsq(double[][] x, double[] b, int[] col,
                   double[] normsq) throws IOException {
        ...
        for (int k = 0; k < m; k++) {
            ...
            if (VERBOSE)
                System.out.print(
                                "  残差２乗和 = " + 
                                RVector.innerProduct(r + 1, x[m], x[m])
                            );
            r++;
        }
        for (int j = r - 1; j >= 0; j--) {
     ...
     */
    static int lsq(double[][] x, double[] b, int[] col,
                   double[] initnormsq, double[] normsq) {
        final int n = x[0].length, m = b.length;
        for (int j = 0; j < m; j++) {
            col[j] = j;
            normsq[j] = RVector.innerProduct(x[j], x[j]);
        ...
        for ( ; r < m; r++) {
            if (r != 0) {
                int j = r; double u = 0.0;
                for (int i = r; i < m; i++) {
                    final double t = normsq[i] / initnormsq[i];
                    if (t > u) { u = t; j = i; }
                }
                //  Java にもクラス・関数テンプレートが欲しい…。
                int k = col[j]; col[j] = col[r]; col[r] = k;
                double s = normsq[j]; normsq[j] = normsq[r]; normsq[r] = s;
                s = initnormsq[j]; initnormsq[j] = initnormsq[r];
                initnormsq[r] = s;
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < m; i++) x[i + con] = raw_x[i];
        if (con != 0) {
            x[0] = new double[n];
        ...
        double[] b = new double[p],         //  回帰係数
                 ...
                 initnormsq = question("ピボット選択を行いますか") ?
                                new double[p] : null;   //  初期ノルム２乗
