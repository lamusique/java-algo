 ...
 *
 * @version $Revision: 1.6 $, $Date: 2003/03/23 15:32:32 $
 */
...
public class Contain {
    ...
    public static void markContained(Interval[] a, boolean[] contained) {
        ...
        for (int i = 1; i < a.length; i++) {
            if (a[i].right <= maxRight) contained[i] = true;
            else {
                maxRight = a[i].right;  contained[i] = false;
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        while (i < N) {
            ...
            if (x < y) { a[i] = new Contain.Interval(x, y);  i++;  }
        }
