 ...
 * @version $Revision: 1.3 $, $Date: 2002/12/16 01:03:27 $
*/
import java.io.*;
import java.util.*;
...
public class RadixConversion {
    ...
    // 桁数(0 でない先頭の桁位置)を保持させればよいが，複雑になるためやっていない。
        ...
        IntArray(int radix, int size) {
            this.radix = radix;   c = new int[this.size = size];
        }
        void init() { Arrays.fill(c, 0); } // 0 で初期化
    }

    static int longToArray(long x, IntArray y) {
        int i, d = y.radix;

        y.init();
        for (i = 0; x != 0 && i < y.size; i++) {
            y.c[i] = (int)(x % d);  x /= d;
        }
        if (x == 0) return i;   /* 桁数 */
        else        return -1;  /* エラー */
    ...
    static int arrayToArray(IntArray src, IntArray dst) { // src の内容は壊される
        dst.init();
        int i, ms = src.size, md = dst.size,
            rs = src.radix, rd = dst.radix;
    ...
    */
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (m1 < 0) {
            System.out.println("longToArray: error");  System.exit(-1);
        }
        ...
        if (m2 < 0) {
            System.out.println("arrayToArray: error");  System.exit(-1);
        }
