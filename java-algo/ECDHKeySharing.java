/*
 *  楕円曲線を用いた鍵交換
 *  @version $Revision: 1.8 $, $Date: 2003/04/04 01:43:13 $
 */
...
class ECPoint {
    ...
    void setZero() { isZero_ = true; }
    // E={y^2=x^3+ax+b}上に点(x, y)があるかどうかをチェック
    boolean isValid(BigInteger x, BigInteger y) {
        BigInteger diff;
    ...
    // コンストラクタ
    ECPoint() { x_ = y_ = ZERO; isZero_ = true; }
    ECPoint(BigInteger x, BigInteger y) {
        BigInteger diff;
        x_ = x.mod(p_); y_ = y.mod(p_);
        isZero_ = false;
        if (!isValid(x_, y_)) throw new Error("E上にありません");
    }
...
class ECDHKeySharing {
    // 生成元を一つ固定
    private static final ECPoint P_ = new ECPoint(BigInteger.ONE,
