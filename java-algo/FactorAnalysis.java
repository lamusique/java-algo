 ...
 *  因子分析
 *
 *  @version $Revision: 1.1 $, $Date: 2003/04/05 10:44:10 $
 */
...
class FactorAnalysis {
    ...
    static void factor(int nfac, double[][] r, double[][] q,
                       double[] lambda, double[] work) throws IOException {
        ...
        for ( ; ; ) {
            ...
            for (int j = 0; j < m; j++) for (int k = 0; k < m; k++)
                q[j][k] = r[j][k];
            if (QRDiagonalizer.eigen(q, lambda, work) == false) {
                System.err.println("収束しません");  System.exit(1);
            }
            double s = RVector.innerProduct(nfac, lambda, lambda);
            System.out.println(iter + ": 非対角成分の RMS 誤差 " + 
                               fmtRMS.toString(Math.sqrt(s / (m * (m - 1)))));
            for (int j = 0; j < m; j++) {
                double sum = 0;
        ...
        System.out.println("合計  " + fmte.toString(t) + 
                           "  " + fmtp.toString(s));
        for (int k = 0; k < nfac; k++)
        ...
        for (int j = 0; j < m; j++) {
            System.out.print(fmtf.toString(j + 1) + "  " + 
                             fmte.toString(r[j][j]) + " ");
            for (int k = 0; k < nfac; k++) {
                q[k][j] *= work[k];
