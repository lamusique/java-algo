/*
* BDMult.java -- 多倍長実数の積
* @version $Revision: 1.8 $, $Date: 2003/03/24 03:37:58 $
...
public class BDMult {
    ...
    *@see BITools
    */
    public static BigDecimal bdMult(final BigDecimal a, final BigDecimal b, int precision) {
        if (a.signum() == 0 || b.signum() == 0) return BDTools.ZERO;
        // 有効桁数のビット数。
        int bp = (int)(precision * Math.log(10) / Math.log(2)) + 1;
        BigInteger m = a.unscaledValue(), n = b.unscaledValue(), r;
        int u = bp / 2;
        ...
        if (fm < fn) return bdMult(b, a, precision); // 再帰, a のビット数>=bのビット数にする
        ...
        if (a.signum() * b.signum() < 0) r = r.negate();
