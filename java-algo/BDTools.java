...
* @version $Revision: 1.15 $, $Date: 2003/03/26 14:35:33 $
* 本文には出力関係などは載せず。
*/
import java.math.*;
import java.util.*;
...
public class BDTools {
    ...
    */
    public static String toDoubleString(BigDecimal x) {
        if (x.signum() == 0) return new String("0.0");
        int exp = getExp10(x);
    ...
    * @return 10 進表現での出力桁数を返します。
    */
    public static int print(BigDecimal x, int p) {  return print(x, p, true); }
    /**
    ...
    */
    public static String toE_FormString(BigDecimal x, int p) { return toE_FormString(x, p, true); }
    /**
    * BigDecimal x を p 桁区切りの <code><b>[+|-]a.bcd...E[+|-]xxx</b></code> 形式の文字列に変換します。
    ...
    */
    public static String toE_FormString(BigDecimal x, int p, boolean newLine) {
        ...
        int decLen = r.length;
        StringBuffer sb = new StringBuffer(decLen); // 自動的に容量が増加
        int i = 0;

        do {    // 負符号，整数部，小数点
            sb.append(r, i, 1);     decLen--;
        } while (r[i++] != '.');
        ...
        for (; i < iMax; i += p) {
            sb.append(r, i, p); sb.append(DELMT);
        }
        if (iRem != 0){
            sb.append(r, i, iRem);  i += iRem;
        }
        decNumber = i - notNum;
