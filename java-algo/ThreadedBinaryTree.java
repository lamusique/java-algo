 ...
 *  ひも付き2分木
 *
 *  @version $Revision: 1.4 $, $Date: 2003/04/28 23:23:20 $
 */
...
public class ThreadedBinaryTree {
    class Node {  // 木のノード
        ...
        private void insertLeft(String key) {  // このノードの左に挿入
            Node q = new Node(key,        // 新しいノードを生成
                     left, isLeftThread,  // 左の子は親の左の子を受け継ぐ
                     this, true);         // 右の子は親を指すひも
            left = q;                // {\tt q} を親（このノード）の左の子にする
            isLeftThread = false;    // {\tt q} はひもでない
        }

        private void insertRight(String key) {  // このノードの右に挿入
            Node q = new Node(key, this, true, right, isRightThread);
            right = q;  isRightThread = false;  // 上の {\tt insertLeft()} 参照
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (input.nextToken() != input.TT_EOF)
            if (input.ttype == input.TT_WORD)  // 標準入力から
