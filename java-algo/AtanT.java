...
* AtanT.java
* arctan(x) を求める。
* @version $Revision: 1.14 $, $Date: 2003/03/05 16:28:04 $
*/
...
public class AtanT {
    ...
    */
    ...
    // r + arctan(u) = arctan(x) を求める。
        ...
        for (int i = 1;; i += 2) {  // このループは３回程度で終わる。
            double last = r;  r += u / i;  if (r == last) break;
            u *= u2;
        }
    ...
    */
    public static void main(String[] args){
        ...
        System.out.println("Atan.atan(x)  - Math.atan(x) = " +
                           (Atan.atan(x)  - Math.atan(x)));
        System.out.println("AtanT.atan(x) - Math.atan(x) = " +
                           (AtanT.atan(x) - Math.atan(x)));
        System.out.println("----------");
