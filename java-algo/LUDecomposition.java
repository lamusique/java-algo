...
* @version $Revision: 1.7 $, $Date: 2003/03/13 14:24:33 $
...
public class LUDecomposition {
    static double lu(double[][] a, int[] ip) {
        ...
        for (int k = 0; k < n; k++) {   // 各行について
            ...
            for (int j = 0; j < n; j++) {
                double t = Math.abs(a[k][j]);  if (t > u) u = t;
            }
        ...
        for (int k = 0; k < n; k++) {   // 各行について
            ...
            for (int i = k; i < n; i++) {// より下の各行について
                ...
                if (t > u) {  u = t;  m = i;  } // 初回は u = -1 && t >= 0 であるから{}は必ず実行される。
            }
            int ik = ip[m];
            if (m != k) {
                ip[m] = ip[k];  ip[k] = ik; // 行番号を交換
    ...
    */
    public static void main(String[] args) throws IOException {
        ...
        double[]   b = new double[n],    bSave = new double[n],
                   x = new double[n];
        ...
        for (int i = 0; i < n; i++) {
            ...
            System.out.println(fmtd.toString(i) + ":" + fmtf.toString(x[i]) +
                fmte.toString(s));
        }
    }
}
