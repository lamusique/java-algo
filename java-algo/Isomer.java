/**
* Isomer.java -- 異性体の問題
* @version $Revision: 1.5 $, $Date: 2002/11/18 06:12:41 $
*/
/**
*異性体の問題を解きます。
*/
...
//        n = size[0] = length[0] = 0;
        for (int i = 0; i < L; i++) {
            ...
            int si = size[i] + 1;  if (si + len > C) continue;
            for (int j = 0; j <= i; j++) {
                int sj = si + size[j];  if (sj + len > C) continue;
                for (int k = 0; k <= j; k++) {
                    int sk = sj + size[k];  if (sk + len > C) continue;
        ...
        for (int i = 0; i <= n; i++) {
            ...
            for (int j = 0; j <= i; j++) {
                ...
                int sj = si + size[j];  if (sj > C) continue;
                count[sj]++;  // 偶数
                for (int k = 0; k <= j; k++) {
                    int sk = sj + size[k] + 1;  if (sk > C) continue;
                    for (int h = 0; h <= k; h++) {
                        int sh = sk + size[h];
                        if (sh <= C) count[sh]++;  // 奇数
                    }
        ...
        for (int i = 1; i <= C; i++)
            System.out.println("炭素原子が " + i + " 個のものは "
                                                + count[i] + " 種類");
    }
}
