 ...
 *
 * @version $Revision: 1.1 $, $Date: 2003/03/20 11:31:06 $
 */
...
public class MultiPrecision {
    ...
    */
    public  static final int RADIXBITS = 30; // 基数のビット数
    /**
    *基数です。2<sup>30</sup> = 1,073,741,824 にとっています。
    */
    public  static final int RADIX = (1 << RADIXBITS); // 基数 1073741824
    private static int n;              // RADIX 進法で小数第 n 位まで
    /**
    *コンストラクタ
    *@param n RADIX 進法の桁数
    */
    ...
    */
    public static void muls(int[] a, int x, int[] b) {
        long t = 0;
        for (int i = n; i >= 0; i--) {
            t += (long)a[i] * x;
            b[i] = (int)t & (RADIX - 1);
    ...
    */
    public static int divs(int m, int[] a, int x, int[] b) {// a[m..0] / x --> b
        long t = 0;
        for (int i = m; i <= n; i++) {
            t = (t << RADIXBITS) + a[i];
            b[i] = (int)(t / x);
...
//          t %= x;
    ...
    */
    public static String toString(int[] r) {
        ...
        StringBuffer x = new StringBuffer(a[0] + ".");

        for (int i = 0; i < n; i++) {
            a[0] = 0;
            muls(a, 1000000000, a);
            StringBuffer sb = new StringBuffer(Integer.toString(a[0]));
            x.append("000000000".substring(0, 9 - sb.length()) + sb);
        }
        return x.toString();
    }
    /**
    *BigDecimal 数値に変換します。
    */
    public static BigDecimal toBigDecimal(int[] r) {
    ...
    *全要素を 0 で初期化します。
    */
    public static void setZero(int[] a) {  Arrays.fill(a, 0); }
