 ...
 *
 *  @version $Revision: 1.9 $, $Date: 2003/10/30 19:53:06 $
 */
public class CMatrix {
    ...
    public static void swapRow(Complex[][] mtx, int i, int j) {
        Complex[] temp = mtx[i]; mtx[i] = mtx[j]; mtx[j] = temp;
    }
    /**
    ...
    public static void swapCol(Complex[][] mtx, int i, int j) {
        for (int k = 0; k < mtx.length; k++) {
            Complex temp = mtx[k][i]; mtx[k][i] = mtx[k][j]; mtx[k][j] = temp;
        }
    }
    /**
     *  転置行列を返します。
    ...
    public static void main(String[] args) {
        ...
        Complex[][]  m2 = (Complex[][])m1.clone(); // 要素はコピーされない！
        Complex[][]  m3 = add(m1,m2), m4 = multiply(m2,m3),
                     m5 = subtract(m3,m4);
