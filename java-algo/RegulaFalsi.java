 ...
 *
 *  @version $Revision: 1.7 $, $Date: 2003/04/05 15:10:00 $
 */

public class RegulaFalsi {
    /** 零点を求めたい関数 f(x) */
    public interface Function {  public double of(double x);  }
    ...
    public static double solve(Function f, double a, double b,
        ...
        double fa = f.of(a);  if (fa == 0) return a;
        double fb = f.of(b);  if (fb == 0) return b;
        if (isSameSign(fa, fb))
            throw new Error("区間の両端で関数値が同符号です。");
        for (int i = 0; ; i++) {
            double c = (a + b) / 2;
            if (c - a <= tolerance || b - c <= tolerance) return c;
            if (i < iMax) {
                c = (a * fb - b * fa) / (fb - fa);
                if      (c <= a) c = a + 0.6 * DBL_EPSILON * Math.abs(a);
                else if (c >= b) c = b - 0.6 * DBL_EPSILON * Math.abs(b);
            }
            double fc = f.of(c);  if (fc == 0) return c;
            if (isSameSign(fc, fa)) {  a = c;  fa = fc;  }
            else                    {  b = c;  fb = fc;  }
        }
    }
