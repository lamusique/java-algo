...
public class Ibeta extends FuncViewer {
    public static double p_beta(double x, double a, double b) { // I_x(a,b)
        ...
        for (int k = 0; k < 200; ) {
            ...
            if (q2 == 0) {
                p2 = Double.POSITIVE_INFINITY; continue;
            }
    ...
    public static double q_beta(double x, double a, double b) { // 1 - I_x(a,b)
        return 1 - p_beta(x, a, b);
    }

    public static double p_t(double t, int df) { // t 分布の下側確率
        return 1 - 0.5 * p_beta(df / (df + t * t), 0.5 * df, 0.5);
    }

    public static double q_t(double t, int df) { // t 分布の上側確率
        return 0.5 * p_beta(df / (df + t * t), 0.5 * df, 0.5);
    }

    public static double p_f(double f, int df1, int df2) { // F 分布の下側確率
        if (f <= 0) return 0;
        return p_beta(df1 / (df1 + df2 / f), 0.5 * df1, 0.5 * df2);
    }

    public static double q_f(double f, int df1, int df2) { // F 分布の上側確率
        if (f <= 0) return 1;
        return p_beta(df2 / (df2 + df1 * f), 0.5 * df2, 0.5 * df1);
    }

    public static double p_binomial(int n, double p, int k) { // 二項分布 B(n,p) の下側（X <= k）の確率
        if (k <  0) return 0;
        if (k >= n) return 1;
        return p_beta(1 - p, k + 1, n - k);
    }

    public static double q_binomial(int n, double p, int k) { // 二項分布 B(n,p) の上側（X <= k）の確率
        if (k <= 0) return 1;
        if (k >  n) return 0;
    ...
    public String getName() { return "Ix(0.5,0.5)"; }
    public double getValue(double x) { return p_beta(x,0.5,0.5); }
}
