/**
* PrinComp.java -- 主成分分析
* @version $Revision: 1.7 $, $Date: 2003/03/13 14:24:34 $
* QR.java -- QR法 を使う。
*/
import java.io.*;
...
public class PrinComp {
    static void analyze(double[][] x, double[][] q, double[] lambda,
            int method) {
        int m = x.length, n = x[0].length;  // n 行 m 列(x[m][n])
        int ndf = n - ((method != 0) ? 1 : 0);   /* 自由度 */
        System.out.println("変数    平均値     " +
            ((method == 0) ? "RMS" : "標準偏差"));
        ...
        for (int j = 0; j < m; j++) {
            ...
            t /= n;
            if (method != 0) for (int i = 0; i < n; i++) x[j][i] -= t;
            q[j][j] = RVector.innerProduct(x[j], x[j]) / ndf;
            double s = Math.sqrt(q[j][j]);
            System.out.println(fmt3d.toString(j + 1) +
                fmt12.toString(t) + fmt12.toString(s));
            if (method == 2) {
                q[j][j] = 1;
        ...
        System.out.println("合計   " + fmt8.toString(t) +
            fmt6.toString(s));
        System.out.println("\n変数  重み");
        for (int j = 0; j < m; j++) {
    ...
    */
    public static void main(String[] args) throws IOException {
        ...
        int n = dataFile.numberOfRows(), m = dataFile.numberOfColumns();  // n 行 m 列

        double[][] x = new double [m][n], q = new double[m][m];
