...
* @version $Revision: 1.10 $, $Date: 2003/05/12 10:24:55 $
* BigDecimal x を
* double "mant * 10^exp"
...
public class BDtoE_Form {
    ...
    * 仮数部を double 値で返します。
    */
    public double mant() { return mant; }
    /**
    * 仮数部を BigDecimal 値で返します。下位に 0 がある場合は切り捨てられます。
    */
    public BigDecimal bdMant() { return bdMant; }
    ...
    */
    public BDtoE_Form(BigDecimal x) {
        if (x.signum() == 0) {
            exp = 0;    mant = 0.0;  bdMant = BDTools.ZERO; return;
        }
        exp = BDTools.getExp10(x);  mant = BDTools.mant;
