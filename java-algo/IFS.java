 ...
 *
 *  @version $Revision: 1.7 $, $Date: 2003/03/19 12:11:12 $
 */
...
public class IFS extends DrawOneByOnePanel {
    ...
    public static class Affine implements Comparable {
        final private double a, b, c, d, e, f, prob;
        public Affine(double a, double b, double c,
    ...
    private Affine[] makeTable(Affine[] affine) {
        double sum = 0;  // 確率の合計
        for (int i = 0; i < affine.length; i++) sum += affine[i].prob;
        java.util.Arrays.sort(affine);  // prob の小さいもの順に整列

        Affine[] table = new Affine[affine.length * 25];  // 表作成
        int r = table.length;
        for (int i = 0; i < affine.length; i++) {
            int k = (int)Math.round(r * affine[i].prob / sum);
            sum -= affine[i].prob;
            do {  table[--r] = affine[i];  } while (--k > 0);
        }
        return table;
    }
