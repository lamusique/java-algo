public class Ci extends FuncViewer {
    private static double EULER = 0.577215664901532860606512090082; // Euler の定数 γ
    ...
    private static double asympt(double x) { // 漸近展開
        ...
        return 0.5 * ((fmax + fmin) * Math.sin(x)
                    - (gmax + gmin) * Math.cos(x));
    }
    ...
    public String getName() { return "Ci(x)"; }
    public double getValue(double x) { return valueOf(x); }
}
